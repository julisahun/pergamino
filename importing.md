...
The correct instruction to the DM is: **rebuild each character in the character
creator** (walk the wizard, export the `.json`, drop it in `players/`). It
takes a few minutes per character, it cannot produce an illegal build, and the
exported file is exactly what this app wants.

The creator is a separate tool and no longer lives in this repo; ask the DM
where their copy is. This app never generates a player file and never asks you
to.
...
```bash
node check-campaign.js <campaign-folder>
```
